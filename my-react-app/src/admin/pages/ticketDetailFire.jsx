...
import { adminReplyTicketFire, adminUpdateTicketStatusFire } from "../api/services";
...
const parseDate = (val) => val?.seconds ? new Date(val.seconds * 1000) : val?.toDate ? val.toDate() : val;

const TicketDetailFire = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const unsub = onSnapshot(doc(db, "tickets", id), (snap) => {
            if (!snap.exists()) { navigate("/admin/ticket-fire"); return; }
            setTicket({ id: snap.id, ...snap.data() });
            setLoading(false);
        }, () => { navigate("/admin/ticket-fire"); setLoading(false); });
        return () => unsub();
    }, [id]); // eslint-disable-line

    useEffect(() => {
        if (ticket?.replies) scrollToBottom();
    }, [ticket?.replies]);
    ...
    const handleReply = async (e) => {
        ...
        setReplying(true);
        try {
            await adminReplyTicketFire(ticket.id, replyMsg);
            setReplyMsg("");
    ...
    const handleStatusUpdate = async (status) => {
        try {
            await adminUpdateTicketStatusFire(ticket.id, status);
        } catch (err) { console.error(err); }
    };
            ...
            <div className="admin-ticket-detail-wrap">
                <div className="admin-ticket-main">
                    <div className="admin-card" style={{ flexDirection: "column", alignItems: "flex-start", gap: 12 }}>
                        ...
                        <div className="admin-ticket-meta">
                            <span><FiUser size={13} /> {t.userName}</span>
                            <span><FiClock size={13} /> {fmtFull(parseDate(t.createdAt))}</span>
                                    ...
                                    <div key={idx} className={`admin-reply-wrapper ${isSender ? "admin-reply-wrapper--sender" : "admin-reply-wrapper--receiver"}`}>
                                        <div className={`admin-reply-bubble ${isSender ? "admin-reply-bubble--sender" : "admin-reply-bubble--receiver"}`}>
                                            <div className="admin-reply-header">
                                                <strong>{isSender ? "Support Team (You)" : t.userName}</strong>
                ...
                <div className="admin-ticket-side">
                    <div className="admin-card" style={{ flexDirection: "column", alignItems: "flex-start", gap: 10 }}>
                        ...
                        <div className="admin-ticket-info-row"><label>Name</label><span>{t.userName}</span></div>
                        <div className="admin-ticket-info-row"><label>Email</label><span>{t.userEmail}</span></div>
                        <div className="admin-ticket-info-row"><label>Opened</label><span>{fmt(parseDate(t.createdAt))}</span></div>
...
export default TicketDetailFire;
